...
func formatColor(c *Color, format ColorFormat, plain bool, commas bool, spaces bool) string {
	...
	case FormatHSLCSS:
		...
		hslCSS := fmt.Sprintf("hsl(%vdeg %v%% %v%%", c.HSL[0], c.HSL[1], c.HSL[2])
		if c.Alpha != nil {
			hslCSS += fmt.Sprintf(" / %s", formatAlpha(*c.Alpha))
		}
		hslCSS += ")"
		if plain {
			workingString = hsl
		} else {
			workingString = hslCSS
		}
	case FormatHSLArray:
	...
	case FormatRGBCSS:
		...
		// CSS format uses spaces, not commas
		rgbCSS := fmt.Sprintf("rgb(%d %d %d", c.RGB[0], c.RGB[1], c.RGB[2])
		if c.Alpha != nil {
			rgbCSS += fmt.Sprintf(" / %s", formatAlpha(*c.Alpha))
		}
		rgbCSS += ")"
		if plain {
			workingString = rgb
		} else {
			workingString = rgbCSS
		}
	case FormatRGBArray:
	...
	case FormatANSI:
		if c.Alpha != nil {
			workingString = fmt.Sprintf("%d;%d;%d;%s", c.RGB[0], c.RGB[1], c.RGB[2], formatAlpha(*c.Alpha))
		} else {
			workingString = fmt.Sprintf("%d;%d;%d", c.RGB[0], c.RGB[1], c.RGB[2])
		}
	default:
		if plain {
			workingString = c.Hex
	...
	// Don't modify commas/spaces for ANSI format - it uses semicolons
