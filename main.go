...
func main() {
	...
	resolvedFormat, forcePlain := resolveFormatAlias(cfg.Format)
	cfg.Format = resolvedFormat
	cfg.Plain = cfg.Plain || forcePlain
	cfg.Commas = !*noCommas
	cfg.Spaces = !*noSpaces
...
func resolveFormatAlias(format string) (string, bool) {
	switch format {
	case "hex-ns":
		return "hex", true
	case "rgb-ansi":
...
func findTemplate(args []string) (string, error) {
	switch len(args) {
	case 0:
		return findTemplateInCurrentDir()
	case 1:
		return args[0], nil
	default:
...
func findTemplateInCurrentDir() (string, error) {
